...
export async function POST(request) {
  try{
    ...
    if (user.count > 0 ){
      const result =  user.items[0].members.find(item => item.code === body.code)
      if (result && result.password && (await bcrypt.compare(body.password, result.password))) {
        ...
        return NextResponse.json(resultUser);
         ...
         const users = user.items[0].members.filter(item => item.id !== result.id)
